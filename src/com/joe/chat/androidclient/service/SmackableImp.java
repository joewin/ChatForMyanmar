...
import com.joe.chat.androidclient.util.LogConstants;
...
public class SmackableImp implements Smackable {
	...
	static void registerSmackProviders() {
		...
		ServiceDiscoveryManager.setIdentityName(ChatForMyanmar.XMPP_IDENTITY_NAME);
		ServiceDiscoveryManager.setIdentityType(ChatForMyanmar.XMPP_IDENTITY_TYPE);
	}
	...
	public SmackableImp(YaximConfiguration config,
		...
		try {
			...
			sc.init(null, new X509TrustManager[] { ChatForMyanmar.getApp(service).mMTM },
	...
	public boolean doConnect(boolean create_account) throws ChatMMException {
	...
	public void addRosterItem(String user, String alias, String group)
			throws ChatMMException {
		tryToAddRosterEntry(user, alias, group);
	}

	public void removeRosterItem(String user) throws ChatMMException {
	...
	public void renameRosterItem(String user, String newName)
		...
		if (!(newName.length() > 0) || (rosterEntry == null)) {
			throw new ChatMMException("JabberID to rename is invalid!");
		}
	...
	private void tryToConnect(boolean create_account) throws ChatMMException {
		try {
			...
			if (!mXMPPConnection.isConnected()) 
			{
				throw new ChatMMException("SMACK connect failed without exception!");
			}
			...
			throw new ChatMMException(e.getLocalizedMessage(), e.getWrappedThrowable());
		} catch (Exception e) {
			// actually we just care for IllegalState or NullPointer or XMPPEx.
			Log.e(TAG, "tryToConnect(): " + Log.getStackTraceString(e));
			throw new ChatMMException(e.getLocalizedMessage(), e.getCause());
		}
	}
	...
	private void tryToMoveRosterEntryToGroup(String userName, String groupName)
		...
		if (groupName.length() == 0)
			...
			try {
				...
				throw new ChatMMException(e.getLocalizedMessage());
			}
		}
	}
	...
	private void tryToRemoveUserFromGroup(RosterGroup group,
			RosterEntry rosterEntry) throws ChatMMException {
		try {
	...
	private void tryToRemoveRosterEntry(String user) throws ChatMMException {
		mRoster = mXMPPConnection.getRoster();
		try {
			RosterEntry rosterEntry = mRoster.getEntry(user);
